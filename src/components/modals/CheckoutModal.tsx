...
export function CheckoutModal({ table, onClose }: { table: TableType, onClose: () => void }) {
  ...
  const canApplyDiscount = can(currentSeller, 'applyDiscount', settings.permissionProfiles);
  const canEditServiceFee = can(currentSeller, 'editServiceFee', settings.permissionProfiles);
  const canLaunchPayment = can(currentSeller, 'launchPayment', settings.permissionProfiles);
  const canChangePaymentMethod = can(currentSeller, 'changePaymentMethod', settings.permissionProfiles);
  const canSplitPayment = can(currentSeller, 'splitPayment', settings.permissionProfiles);
  const canCancelPayment = can(currentSeller, 'cancelPayment', settings.permissionProfiles);
  ...
  const discountAmountValue = roundMoney(Math.min(subtotal + feeValue, Math.max(0, rawDiscountAmount)));
  ...
  const handleAddPayment = () => {
    ...
    if (payments.length > 0 && !canSplitPayment) return;
                 ...
                 <div className="grid grid-cols-4 gap-4 mb-6">
                    {[
                      ...
                      <button 
                        ...
                        className={`p-6 rounded-2xl border transition-all flex flex-col items-center gap-3 disabled:opacity-20 disabled:cursor-not-allowed ${currentMethod === m.id ? 'bg-primary border-primary shadow-2xl shadow-primary/20 scale-105' : 'glass border-white/5 opacity-50 hover:opacity-100'}`}
                      >
                        <m.icon size={26} />
