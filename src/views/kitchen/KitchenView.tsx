...
function KitchenOrderCard({ order, onClick }: { order: any, onClick: () => void }) {
  ...
  return (
    <motion.div 
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`p-6 cursor-pointer relative border-[3px] rounded-[2.5rem] transition-all duration-300 h-full flex flex-col shadow-lg ${
        isDanger ? 'bg-rose-500 border-rose-600 text-white shadow-rose-200' : 
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <div className={`flex items-center gap-1 font-black ${isDanger ? 'text-white' : isWarning ? 'text-black' : 'text-primary'}`}>
          <Clock size={18} />
      ...
      <div className="flex-1">
        <p className={`text-xs font-black uppercase tracking-widest mb-3 ${isDanger ? 'text-white/60' : 'text-black/40'}`}>{order.items.length} Itens</p>
        <div className="space-y-2">
          {order.items.slice(0, 3).map((item: any, idx: number) => (
            <div key={idx} className="flex gap-2 items-center">
              <span className={`font-black text-lg ${isDanger ? 'text-white' : 'text-primary'}`}>{item.quantity}x</span>
              <span className="font-bold text-lg truncate">{item.name}</span>
            </div>
          ))}
          {order.items.length > 3 && (
            <p className={`text-xs font-bold ml-8 ${isDanger ? 'text-white/40' : 'text-black/30'}`}>+ {order.items.length - 3} outros itens...</p>
          )}
        </div>
      </div>

      <div className={`mt-4 pt-4 border-t flex justify-between items-center ${isDanger ? 'border-white/20' : 'border-black/5'}`}>
        <span className={`text-[10px] font-black uppercase tracking-widest ${isDanger ? 'text-white/40' : 'text-black/30'}`}>Clique para abrir</span>
        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${isDanger ? 'bg-white text-rose-600' : isWarning ? 'bg-black text-amber-400' : 'bg-primary/10 text-primary'}`}>
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="w-full max-w-5xl bg-white rounded-[3rem] shadow-3xl overflow-hidden flex flex-col max-h-[90vh]"
      >
        <div className="p-10 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex items-center gap-8">
             ...
             <div>
                ...
                <p className="text-primary font-black uppercase tracking-[0.3em] text-sm">Preparando Agora</p>
             </div>
          </div>
          <button onClick={onClose} className="p-6 bg-gray-100 rounded-full hover:bg-rose-50 text-rose-500 transition-all">
            <X size={40} />
          </button>
        </div>
                ...
                <div key={idx} className="p-8 bg-gray-50 rounded-[2.5rem] border border-gray-100">
                   <div className="flex justify-between items-start">
                      <div className="flex items-center gap-6">
                         <div className="w-16 h-16 bg-white shadow-sm border border-gray-100 rounded-2xl flex items-center justify-center text-3xl font-black text-primary">
                           {item.quantity}x
                         </div>
                   ...
                   {item.selectedModifiers && item.selectedModifiers.length > 0 && (
                     <div className="mt-6 flex flex-wrap gap-3 ml-20">
                        {item.selectedModifiers.map((m: any, mIdx: number) => (
                          <span key={mIdx} className="px-5 py-2 bg-primary/10 text-primary rounded-xl text-sm font-black uppercase tracking-widest">
                            + {m.name}
                          </span>
                        ))}
                     </div>
        ...
        <div className="p-10 bg-gray-50 border-t border-gray-100">
           <button 
             ...
             className="w-full py-8 bg-emerald-500 hover:bg-emerald-600 text-white rounded-[2rem] text-3xl font-black uppercase tracking-widest shadow-xl shadow-emerald-200 transition-all active:scale-[0.98] flex items-center justify-center gap-4"
           >
            ...
            >
              <div className="text-center max-w-2xl">
                 ...
                 <div className="grid grid-cols-2 gap-6">
                    <button 
                      onClick={onComplete}
                      className="py-8 bg-emerald-500 text-white rounded-[2rem] text-xl font-black uppercase tracking-widest shadow-lg"
                    >
                      Sim, finalizar
                    </button>
                    <button 
                      onClick={() => setShowConfirm(false)}
                      className="py-8 bg-gray-100 text-gray-500 rounded-[2rem] text-xl font-black uppercase tracking-widest"
                    >
                      Não, voltar
                    </button>
                 </div>
...
export function KitchenView() {
  ...
  return (
    <div className="p-6 bg-white h-screen text-black font-['Outfit'] overflow-hidden flex flex-col">
      <div className="flex justify-end items-center mb-6">
        <div className="bg-gray-100 px-6 py-2 rounded-2xl border border-gray-200 flex items-center gap-3">
           <div className="w-3 h-3 bg-primary rounded-full animate-pulse shadow-[0_0_10px_rgba(139,92,246,0.5)]" />
