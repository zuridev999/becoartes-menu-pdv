...
const ZURI_WHATSAPP_BRIDGE_HOST_HEADER = process.env.ZURI_WHATSAPP_BRIDGE_HOST_HEADER || '';
if (
...
const {
  ...
  zuriWhatsApp: {
    ...
    bridgeHostHeader: ZURI_WHATSAPP_BRIDGE_HOST_HEADER,
  },
});
...
const closeBillWithInventorySync = async (data, session = null) => {
  ...
  const isServiceFeeReduced = serviceFeeCents < defaultServiceFeeCents;
  ...
  if (serviceFeeCents !== defaultServiceFeeCents && !isServiceFeeReduced) {
