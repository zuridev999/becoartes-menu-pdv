...
assert.match(component, /qr-menu-inline/);
assert.match(component, /data-ad-format=\{isCompactTop \? undefined : isQrMenuInline \? 'rectangle' : 'auto'\}/);
...
assert.equal((qrView.match(/afterIntroContent=\{<GoogleAdBanner placement="qr-menu-inline" onViewportVisibilityChange=\{setIsInlineAdVisible\} \/>\}/g) || []).length, 2);
...
assert.match(app, /const isPublicTopAdView = activeView === 'tablet' \|\| activeView === 'delivery'/);
assert.doesNotMatch(app, /activeView === 'qr'[^;\n]*placement="top"/);
