...
const orderId = `delivery_webhook_smoke_${Date.now()}`;
...
const seedCatalog = async () => {
  ...
  await db.execute("INSERT OR IGNORE INTO categories (id, name, sort_order, visible) VALUES ('cat_webhook_smoke', 'Smoke Webhook', 1, 1)");
  await db.execute(`
    INSERT OR REPLACE INTO menu (id, name, description, price, category_id, image, visible, cost)
    VALUES ('produto_webhook_smoke', 'Item Webhook Smoke', 'Produto local de teste webhook', 15, 'cat_webhook_smoke', '', 1, 0)
  `);
};
...
const checkout = await requestJson('/api/delivery/checkout', {
  ...
  body: JSON.stringify({
    ...
    items: [{
      id: `${orderId}_item_1`,
      productId: 'produto_webhook_smoke',
      name: 'Item Webhook Smoke',
      price: 15,
      quantity: 1,
      selectedModifiers: [],
      notes: 'teste webhook',
...
console.log(JSON.stringify({
  ...
  dbUrl,
  orderId,
  initialPaymentStatus: checkout.order.paymentStatus,
